# Code adapted from https://github.com/CoreElectronics/CE-Arducam-MicroPython/tree/main
# Copyright (c) 2023 Core Electronics Pty Ltd., released under MIT license
# Adapted by Silicognition LLC to get JPG from async generator
...
class Camera:
    ...
    def __init__(self, spi_bus, cs, skip_sleep=False, debug_text_enabled=False):
        ...
        if self.camera_idx == '3MP':
            time.sleep_ms(self.WHITE_BALANCE_WAIT_TIME_MS)# fifo length is sometimes a broken value (5 to 8 megabytes) and causes program to fail this helps 
            self._wait_idle()
            self.startup_routine_3MP()
        if self.camera_idx == '5MP' and skip_sleep == False:
            time.sleep_ms(self.WHITE_BALANCE_WAIT_TIME_MS)
    ...
    def capture_jpg(self):
        if (time.ticks_diff(time.ticks_ms(), self.start_time) <= self.WHITE_BALANCE_WAIT_TIME_MS) and self.camera_idx == '5MP':
    ...
    async def get_JPG(self):
        ...
        while recv_len > 0:
            ...
            if inx >= 0:
                yield self.image_buffer[:inx+2]
                break
            elif last_byte == 0xff and self.image_buffer[0] == 0xd9:
                yield b'\xd9'
                break
            else:
                yield self.image_buffer
        self.cs.on()
